...
# $email is a valid address as defined by RFC2821.
# Good syntax extract can be found at http://en.wikipedia.org/wiki/Talk:E-mail_address
# Except: 
#   Only thing not checked is length of domainPart
...
#old pattern "^[\&\'-_.[:alnum:]]+@((([[:alnum:]]|[[:alnum:]][[:alnum:]-]*[[:alnum:]])\.)" 

	$atext = "[a-zA-Z0-9!#$%&'*+\-\/=?^_`{|}~]";
	$dotString = "$atext(\.?$atext){0,63}";
	$qtext = "[\\x01-\\x08\\x0b\\x0c\\x0e-\\x1f\\x21\\x23-\\x5b\\x5d-\\x7e\\x7f]";
	
	$quotedPair = "\\\\[\\x01-\\x0a\\x0c\\x0e-\\x7f]";
	$qcontent = "$qtext|$quotedPair";
	$quotedString = "\"( |\t|$qcontent){1,62}\""; // The extreemly complicated construct in the RFC can be rewritten to this, I think, (when ignoring the folding mechanism)
	...
	$domainPart = "[a-zA-Z0-9](-?[a-zA-Z0-9])*(\.[a-zA-Z](-?[a-zA-Z0-9])*)*\.$topLevelDomain";
	$validEmailPattern = "/^$localPart@$domainPart$/"; 
